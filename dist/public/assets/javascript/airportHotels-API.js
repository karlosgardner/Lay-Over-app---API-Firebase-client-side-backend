    ...
    function amadeus(url) {
        $.ajax({
            ...
            for (var i = 0; i < hotels.length; i++) {
                ...
                myObject.bed = Array.isArray(hotels[i].rooms) && hotels[i].rooms.length > 0 && hotels[i].rooms[0].room_type_info && hotels[i].rooms[0].room_type_info.bed_type || "N/A", myObject.image = Array.isArray(hotels[i].image) && hotels[i].image.length > 0 && hotels[i].image[0] || "N/A";

                $("#textArea").append(myObject.name + "<br>" + myObject.address + "<br>" + myObject.city + ", " + myObject.state + ", " + myObject.zip + "<br>" + myObject.country + "<br><span><strong>The price of stay: $" + myObject.price + "</strong></span><br><span>Restaurant and/or internet access at this hotel: " + myObject.amenities + ".</span><br><span>Summary of room info: " + myObject.description + "</span><br><span>Phone number: " + myObject.phone + "</span><br><span>Hotel bed type: " + myObject.bed + "</span><br><span>Hotel rating: " + myObject.rating + "</span><hr>"); //figure out how to write hotel ratings and room type if true to DOM====Answer see myObject for answer
